...
import com.ibiz.api.dao.EmployeeDAO;
...
public class EmployeeService {
    ...
    public JsonObject<List<EmployeeVO>, Object> selectEmployeeInfo(Payload<EmployeeVO> requestPayload) {
        ...
        EmployeeVO userBaseInfoVO = requestPayload.getDto();

        log.info("Paramater : " + accountVO);
        log.info("Paramater : " + userBaseInfoVO);

        List<EmployeeVO> list = employeeDAO.selectEmployeeInfo(userBaseInfoVO);

        jsonObject.Data = list;
        jsonObject.TotalSize = list.size() > 0? list.get(0).getTotalCnt() : 0;
        jsonObject.IsSucceed = true;

        return jsonObject;
    }

    public JsonObject<Object, Object> updateHRIfInfo(Payload<DeptHistoryVO> requestPayload) throws Exception {
        ...
        EmployeeIFVO[] userInfoArry = restTemplate.postForObject(requestUri, parameterMap, EmployeeIFVO[].class);
        //2.2리턴결과 송신
        if(userInfoArry[0].getSND_STAT_CD().trim().equals("1"))
        {
            nResult = 0;

            for(EmployeeIFVO user : userInfoArry)
            {
                EmployeeIFVO userTmp = new EmployeeIFVO();
        ...
        List<EmployeeResultIFVO> userlist = new ArrayList<EmployeeResultIFVO>();
        userlist = hrIFDAO.selectApiUserInfo();

        for(EmployeeResultIFVO user : userlist)
        {
            ...
            EmployeeResultIFVO userTmp = hrIFDAO.selectIsValidUserInfo(user);

            try {

                //1.사원기본
                if(!userTmp.getValChk().trim().equals("0"))
                    nResult = hrIFDAO.updateUserInfo(user);
                else
                    nResult = hrIFDAO.insertUserInfo(user);

                //2.고객관련

                //2.1고객ID 가져오기
                userTmp = hrIFDAO.selectCreateCustomerID(user);
                user.setCustId(userTmp.getCustId() == null || userTmp.getCustId().length() == 0 ? "" : userTmp.getCustId());

                //2.2고객기본
                nResult = hrIFDAO.insertCustomerHistory(user);

                nResult = hrIFDAO.updateCustomerInfo(user);
                if(nResult < 1)
                    nResult = hrIFDAO.insertCustomerInfo(user);

                //2.3고객역활
                nResult = hrIFDAO.updateCustomerRole(user);
                if(nResult < 1)
                    nResult = hrIFDAO.insertCustomerRole(user);

                //2.4고객연락처(사내전화)
                nResult = hrIFDAO.updateCustomerContactNumber(user);
                if(nResult < 1 && user.getCompTelNo() != null && user.getCompTelNo().length() > 0 )
                    nResult = hrIFDAO.insertCustomerContactNumber(user);

                //2.5고객연락처(모바일)
                nResult = hrIFDAO.updateCustomerContactMobileNumber(user);
                if(nResult < 1 && user.getMbphNo() != null && user.getMbphNo().length() > 0)
                    nResult = hrIFDAO.insertCustomerContactMobileNumber(user);

                //2.6고객연락처(이메일)
                nResult = hrIFDAO.updateCustomerContactEmlAdrs(user);
                if(nResult < 1 && user.getEmlAdrs() != null && user.getEmlAdrs().length() > 0)
                    nResult = hrIFDAO.insertCustomerContactEmlAdrs(user);

                //2.7고객관계
                nResult = hrIFDAO.updateCustomerRelation(user);
                if(nResult < 1)
                    nResult = hrIFDAO.insertCustomerRelation(user);
        ...
        //3. 변경이력 저장
        EmployeeChangeIFVO[] ifhrChangeArry;
        ...
        ifhrChangeArry  = restTemplate.postForObject(requestUri, parameterMap, EmployeeChangeIFVO[].class);

        if(ifhrChangeArry[0].getSND_STAT_CD().trim().equals("1"))
        {
            nResult = 0;

            for(EmployeeChangeIFVO hrchg : ifhrChangeArry)
            {
                EmployeeChangeIFVO hrchgTmp = new EmployeeChangeIFVO();
    ...
    @Transactional
    public JsonObject<List<EmployeeVO>, Object> selectEmployeeSearchList(Payload<EmployeeVO> requestPayload) throws Exception {
        log.info("call Service : " + this.getClass().getName() + ".selectEmployeeSearchList");
